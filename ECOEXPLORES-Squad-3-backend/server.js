...
const PERFIS_VALIDOS = new Set(["aluno", "professor", "diretor", "pai"]);
...
app.post("/auth/register", (req, res) => {
  ...
  const perfil = (req.body?.perfil ?? "").toString().trim().toLowerCase();
  ...
  const existe = users.find(u => u.documento === documento);
  if (existe) return res.status(409).json({ mensagem: "Documento já cadastrado" });

  const password_hash = bcrypt.hashSync(senha, 10);
  const novoId = users.reduce((m, u) => Math.max(m, u.id), 0) + 1;

  const novoUsuario = { id: novoId, nome, documento, perfil, password_hash };
  users.push(novoUsuario);

  const token = gerarToken(novoUsuario);
  return res.status(201).json({
    token,
    usuario: { id: novoUsuario.id, nome: novoUsuario.nome, documento: novoUsuario.documento, perfil: novoUsuario.perfil }
  });
});
...
// SERVIR O FRONTEND (VITE BUILDADO)
const frontendPath = path.join(__dirname, "../ECOEXPLORES---Squad-3-main/dist");
